...
async function run() {
  ...
  if (!pr) return core.setFailed("No pull request found in the context.");

  const { owner, repo } = context.repo;
  const [prError, prResult] = await expectError(
    handlePullRequest({ token, owner, repo, prNumber: pr.number })
  );
  if (prError) return core.setFailed(prError.message);

  const { comments } = prResult;
  if (!comments.length) return core.info("No security issues found.");

  const [postError] = await expectError(
    postReviewComment(token, owner, repo, pr.number, comments)
  );
  if (postError) return core.setFailed(postError.message);
}
